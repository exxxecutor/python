...
def extension(files):
    ...
    for file in files:
        ext = file.split('.')[-1]
        if ext in extensionDict:
            extensionDict[ext] += 1
        else:
            extensionDict[ext] = 1

    maxVal = max(extensionDict.values())
    res = []
    for (key, value) in extensionDict.items():
        if value == maxVal: res.append('.' + key)
...
# Тесты
try:
    assert extension(["Lakey - Better days.mp3", "Wheathan - Superlove.wav", "groovy jam.als",
